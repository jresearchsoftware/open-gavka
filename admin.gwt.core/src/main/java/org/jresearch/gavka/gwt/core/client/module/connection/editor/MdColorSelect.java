...
public class MdColorSelect extends Select<String> {
	...
	public MdColorSelect(final String label) {
		super(label, toSelectOprions(EnumSet.allOf(ColorEnum.class)));
	}

	private static List<SelectOption<String>> toSelectOprions(final EnumSet<ColorEnum> colors) {
		return colors.stream().map(ColorEnum::getColor).map(MdColorSelect::toSelectOption).collect(Collectors.toList());
	}

	private static SelectOption<String> toSelectOption(final Color color) {
	...
	public static String any() {
		return anyColor().getName();
	}

	public static Color anyColor() {
